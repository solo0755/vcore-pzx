...
bool ChatHandler::HandlePDumpLoadCommand(char *args)
{
	...
	char* account_name= ExtractLiteralArg(&args);
	if (!account_name)
	{
		return false;
	}
...
bool ChatHandler::HandlePDumpTransCommand(char *args) {

	uint32 count = 0;
	sLog.outString(">> [pzx char trancf]  HandlePDumpTransCommand starting");
	QueryResult* result = CharacterDatabase.PQuery("select c.guid,a.username from  characters c left join account a on a.id = c.account   LIMIT 0, %u",  sPzxConfig.GetIntDefault("characters.trans.numm", 1000));
	if (!result)
	{
		BarGoLink bar(1);
		bar.step();
	...
	do
	{
		...
		if (PlayerDumpWriter().WriteDump(file, guid) == DUMP_SUCCESS)
		{
			...
			if (!fout) {
				//PSendSysMessage(GetSession(), u8"[系统消息]:[%u-%s] 异常", guid, name);
			...
			Player::DeleteFromDB(guid, 0, true, true);
...
bool ChatHandler::HandlePDumpWriteAllCommand(char *args)
{
	...
	char* p1 = ExtractLiteralArg(&args);
	char* p2 = ExtractLiteralArg(&args);
	uint32 acct1;
	uint32 acct2;
	if (!ExtractUInt32(&p1, acct1)) {
		return false;
	}
	if (!ExtractUInt32(&p2, acct2)) {
		return false;
	}

	std::map<uint32,std::string> acctMap;
	QueryResult* result = LoginDatabase.PQuery("select id,username from account where id BETWEEN %u AND %u", acct1, acct2);
	if (!result)
	{
		BarGoLink bar(1);
		bar.step();
	...
	do
	{
		...
		bar.step();
		Field* fields = result->Fetch();
		uint32 accid= fields[0].GetInt32();
		string username = fields[1].GetCppString();
		if (access("dump", 6) == -1) {
#ifdef WIN32
			mkdir("dump");
#else
		...
		username = "dump/" + to_string(accid) ;
		

		acctMap.insert(pair<uint32,string >(accid,username));
	} while (result->NextRow());
	delete result;
	
	 result = CharacterDatabase.PQuery("SELECT guid,account FROM characters where account  BETWEEN %u AND %u", acct1, acct2);
	if (!result)
	{
		BarGoLink bar(1);
		bar.step();
	...
	do
	{
		...
		uint32 accid = fields[1].GetInt32();
	
		string username = acctMap[accid];
		//创建目录
		if (access(username.c_str(), 6) == -1)
		{
		...
		string file = username + "/" + to_string(guid);
		...
		case DUMP_FILE_OPEN_ERROR:
			PSendSysMessage(LANG_FILE_OPEN_FAIL, file);
			sLog.out(LOG_GM_CRITICAL, "ACCOUNT:%u (GUID:%u) [FAILED]", accid, guid);
			SetSentErrorMessage(true);
			break;
		default:
		...
		sLog.outString(">>dump,ACCOUNT:%u (GUID:%u) success", accid, guid);

	/*	std::string dumps = PlayerDumpWriter().GetDump(guid);
		sLog.out(LOG_GM_CRITICAL, "ACCOUNT:%u (GUID:%u) [%s]", accid, guid, dumps.c_str());
		bool re=CharacterDatabase.PExecute("insert into _chara_dump values(`%u`,`%u`,`%s`)", guid, accid, dumps.c_str());
		if (re) {
			sLog.outString(">>dump,ACCOUNT:%u (GUID:%u) success", accid, guid);
		}else
		sLog.outString(">>dump,ACCOUNT:%u (GUID:%u) failed",accid,guid);*/

	} while (result->NextRow());

	delete result;

	sLog.outString("dump %u-%u finsh ", acct1, acct2);
...
std::string GetNameCol(Player* player)
{
...
void PZXSendSysMessage(Player *player, const char *format, ...)
{
	va_list ap;
	char str[2048];
	va_start(ap, format);
...
bool ChatHandler::HandleAddZZCommand(char* args) {
	if (!sPzxConfig.GetIntDefault("openzzcmd", 0)) {
		SendSysMessage(LANG_BAD_VALUE);
		return false;
	}
	if (!*args)
	...
	int32 addZZ = atoi(args);
	if (sPzxMgr->addPoints(addZZ, chr->GetSession()->GetAccountId())) {
		PSendSysMessage(u8"赞助点增加[%d],成功", addZZ);
		PZXSendSysMessage(chr, u8"赞助点增加[%d],成功", addZZ);
	}
	else {
		PSendSysMessage(u8"赞助点增加[%d],失败", addZZ);
	}
	return true;
}
bool ChatHandler::HandleZYCommand(char* args)
{
	try
	{
	...
	if (player&&player->HasItemCount(49999, 1) && player->IsDead()&& player->CustomPlayerActionTimeCheck(sPzxConfig.GetIntDefault("timecool.pullplayer", 15), PLAYED_TIME_PULL)) {
	...
	// Player Muted?
	if (!player->CanSpeak())//公共频道禁言？？
		return false;
	if (!player->IsGameMaster()||!player->HasItemCount(49999, 1)) {//没有道具或者不是GM，需要受频道限制
		if (player->GetTotalPlayedTime() <= sPzxConfig.GetIntDefault("zyChannelInit", 3600 * 3) && !player->IsGameMaster()) {
			PSendSysMessage(u8"需要在线时长3小时以上，才可以使用ZY频道。");
			return false;
		}
		const auto currTime = time(nullptr);
		const auto delta = currTime - GetSession()->GetLastPubChanMsgTime();
		int32 secs = sPzxConfig.GetIntDefault("zyChannelCool", 30);
		if (secs> 30&&delta < secs&& !player->IsGameMaster()) {//30秒以上才生效
				PSendSysMessage(u8"ZY频道使用间隔为%d秒,请稍后再试", secs);
				return false;
			}
	}

	GetSession()->SetLastPubChanMsgTime(time(nullptr));
	// Other Check.
	std::string temp = args;
	...
	switch (player->GetSession()->GetSecurity())
	{
		...
		case SEC_PLAYER:
			...
			{
				msg += u8"|cff0000ff联盟] ";
				msg += GetNameCol(player);
				msg += "|Hplayer:" + name + "|h[" + name + "]|h|r";
				msg += ": |cffb89189";
		...
		case SEC_ADMINISTRATOR:
			msg += u8"|cff9B9B9B 管理者] ";
			msg += GetNameLink(player);
			msg += ": |cffE6CC80";
			break;

	}
		sWorld.LogChat(player->GetSession(), "ZY", args);
		msg += args;
		sWorld.SendWorldText(20000, msg.c_str());
	
		return true;
	}
	catch (...)
	{
		sLog.outError("[pzx-SHE-EXCEPTION]");
		return false;
	}
}

...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password, std::string emailStr)
{
	...
	if (!LoginDatabase.PExecute("update account set email='%s' where username='%s'", email.c_str(), username.c_str()))
	{
		sLog.outError("%s email insert ERROR:%s", username.c_str(), email.c_str());
	}

...
void BattleGroundMgr::CreateInitialBattleGrounds()
{
    ...
    do
    {
		...
		std::string startA = "startA.";
		std::string startAConfig = startA + std::to_string(start1);

		Tokens tokensA = StrSplit(sPzxConfig.GetStringDefault(startAConfig.c_str(), ""), ";");
		float_t tokensAxyz[3];
		int count = 0;
		for (auto& token : tokensA)
		{
			if (token.length() > 2 && count <= 2) {
				try
				{
					tokensAxyz[count] = (atof(token.c_str()));
				}
        ...
        if (start)
        {
            ...
            AStartLoc[0] = fabs(tokensAxyz[0]) > DBL_EPSILON ? tokensAxyz[0]:start->x;
            AStartLoc[1] = fabs(tokensAxyz[1]) > DBL_EPSILON ? tokensAxyz[1]:start->y;
            AStartLoc[2] = fabs(tokensAxyz[2]) > DBL_EPSILON ? tokensAxyz[2]:start->z;
		...
		std::string startB = "startB.";
		std::string startBConfig = startB + std::to_string(start2);
		Tokens tokensB = StrSplit(sPzxConfig.GetStringDefault(startBConfig.c_str(), ""), ";");
		float_t tokensBxyz[3];
		count = 0;
		for (auto& token : tokensB)
		{
			if (token.length() > 2 && count <= 2) {
				try
				{
					tokensBxyz[count] = (atof(token.c_str()));
				}
				catch (const std::exception&)
				{
					sLog.outError("[pzx startB error]:%s", token);
        ...
        if (start)
        {
            HStartLoc[0] = fabs(tokensBxyz[0]) > DBL_EPSILON ? tokensBxyz[0] : start->x;
            HStartLoc[1] = fabs(tokensBxyz[1]) > DBL_EPSILON ? tokensBxyz[1] : start->y;
            HStartLoc[2] = fabs(tokensBxyz[2]) > DBL_EPSILON ? tokensBxyz[2] : start->z;

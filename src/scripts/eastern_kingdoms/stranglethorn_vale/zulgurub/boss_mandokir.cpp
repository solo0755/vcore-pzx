...
enum
{
    ...
    // OK 
    SPELL_FEAR          = 19134, //29544, // 20511 // must affect up to 20 targets.破胆怒吼
    ...
    // OK
    SPELL_WATCH         = 24314,//威慑凝视，血领主曼多基尔正在凝视着你……
    SPELL_DECAPITATE    = 24315, //冲锋
    SPELL_SUMMON_PLAYER = 25104, //召唤玩家，被干死后复活

    // Level-up - Chaque fois que Mandokir tue un joueur, il gagne un niveau.
    SPELL_LEVEL_UP      = 24312,

    // ??
    SPELL_OVERPOWER     = 24407,//"压制",142="立刻压制敌人，对其造成武器伤害
    ...
    SPELL_SUNDERARMOR     = 24317,//奥跟的技能，破甲
    SPELL_THRASH          = 3391, //痛击 使施法者获得$s1次额外的攻击机会
    SPELL_EXECUTE         = 7160,// 斩杀，低于20%发起
...
struct boss_mandokirAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit* pVictim) override
    {
        if (pVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            DoCastSpellIfCan(m_creature, SPELL_LEVEL_UP, true);//杀死玩家后BOSS升级+1
            m_creature->SetLevel(m_creature->GetLevel() + 1);
            m_uiPlayerToRez = pVictim->GetGUID();//设置要复活的玩家

            DoScriptText(SAY_DING_KILL, m_creature);//发送杀人text

            if (!m_pInstance)
                return;

            if (!urand(0,2))//妖术师金度也要来插嘴说句话，看样子是随机的
            {
                if (Creature* jTemp = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(DATA_JINDO)))
                {
                    ...
                    DoScriptText(SAY_GRATS_JINDO, m_creature);//或者血领主说
            }
        }
    }
    ...
    void CheckWatchedPlayer()
    {
        if (m_uiWatchTarget)
        {
            ...
            {
                // Pendant le debuff
                if (pWatchTarget->HasAura(SPELL_WATCH))//记录在凝视期间做的事情，如果有施法、移动、攻击行为都视为秒杀目标
                {
                    ...
                    // Ni attaquer.如果做出了有威胁值的技能，（之前会记录这个技能的仇恨值）
                    if ((!m_bTargetActed) && (m_creature->GetThreatManager().getThreat(pWatchTarget) > m_fTargetThreat))
                        m_bTargetActed = true;
                }
                // Le debuff est termine，buff结束后
                else
                {
                    if (m_bTargetMoved || m_bTargetActed)//目标移动、施法、攻击过都将被定为秒杀目标
                        m_uiTargetToKill = m_uiWatchTarget;
                    else
                        m_uiTargetToKill = 0;
                    m_uiWatchTarget = 0;//buff结束后要重置凝视目标
                }
            }
        }
    }
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        CheckRaptor();//检查奥根是否死亡，死亡就增加激怒BUFF
        CheckWatchedPlayer();
        // Check player to rez
        if (m_uiPlayerToRez)
        {
            if (Player* killedPlayer = m_creature->GetMap()->GetPlayer(m_uiPlayerToRez))
            {
                ...
                if (killedPlayer->GetDeathState() == CORPSE && !killedPlayer->IsRessurectRequested())//尸体状态，而且还没有复活选择框
                {
                    // Find nearest spirit ready to resurrect （被禁锢的灵魂） ready to resurrect（使复活）
        ...
        {
            if (m_creature->IsNonMeleeSpellCasted(false))//如果近战技能还没冷却。等待下一轮
        ...
        {
            if ((m_uiGlobalCooldown == 0) && (!m_uiWatchTarget))//全团没有凝视目标，再寻找一个新目标
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, nullptr, SELECT_FLAG_PLAYER))
                {
                    if (Player* pPlayer = pTarget->GetCharmerOrOwnerPlayerOrPlayerItself())
                    {
                        ...
                        {
                            DoScriptText(SAY_WATCH, m_creature, pTarget);//喊话，我正凝视着你
                            DoScriptText(SAY_WATCH_WHISPER, m_creature, pTarget);//喊话：Don't make me angry. You won't like it when I'm angry.
            ...
            if (m_uiGlobalCooldown == 0)
            {
                if (Unit* pTargetToKill = m_creature->GetMap()->GetUnit(m_uiTargetToKill))
                {
                    if (pTargetToKill->IsAlive())
                    {
                        ...
                        float addAggro = 0;//当前目标对象的仇恨值（一般是T）
                        ...
                        m_creature->GetThreatManager().addThreat(pTargetToKill, (addAggro + 2000));//把目标仇恨值设置为T的仇恨+2000，太BT了，这样即使冲锋死不掉，全团也会因为他OT
                        m_creature->SelectHostileTarget();//关键点在这里，如果凝视期间，T的仇恨不能
        ...
        else
        {
            ...
            if (m_bChargeCasted)
            {
                ...
                if (m_uiChargeCasted_Timer < diff)//2秒后归位
                {
                    m_bChargeCasted = false;
                    m_creature->GetMotionMaster()->MoveChase(m_creature->GetVictim());//冲锋完成后归位，追逐回之前的攻击目标（一般是T）
                }
            }
        }
            ...
            if (m_bFearAfterCharge)
            {
                if(m_uiChargedPlayerGUID)
                {
                    if (Player* player = m_creature->GetMap()->GetPlayer(m_uiChargedPlayerGUID))
                    {
                        if (player->IsAlive())
                        {
                            if(m_creature->GetDistance(player) < 4.0f)//冲锋没搞死玩家就施法恐惧
                            {
                        ...
                        {//如果冲锋搞死了玩家，就重置恐惧时间2400、gcd500，标志位m_bFearAfterCharge=false
                                    m_uiFear_Timer = 24000;
                                    m_uiGlobalCooldown = 500;
                                    m_bFearAfterCharge = false;
                                    m_uiChargedPlayerGUID.Clear();
        ...
        {
            if ((m_uiGlobalCooldown == 0) && (!m_uiTargetToKill) && (!m_uiWatchTarget))//凝视期间或者斩首期间，不可以旋风斩
            {
        ...
        DoMeleeAttackIfReady();//其他时间做近战攻击
    }
};

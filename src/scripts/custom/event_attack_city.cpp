...
struct npc_attack_masterAI : public ScriptedAI
{
...
#ifdef DEBUG_ON
        sLog.outString("Invocateur %u (ang %f) [%f,%f]", num, circleAng, positionX, positionY);
#endif
    ...
    void UpdateEvent_FinalFight(uint32 const uiDiff)
    {
        switch (uiCurrentPhaseStep)
        {
            case 1:
            {
                ...
                if (!cityBoss)
                {
                    sLog.outError("Impossible d'invoquer le maitre de la ville !");
                    uiCD = CD_INFINY;
                    return;
                }
    ...
    void UpdateAI(uint32 const uiDiff) override
    {
        if (uiCD < uiDiff)
        {
            ...
            uiCurrentPhaseStep++;
#ifdef DEBUG_ON
            sLog.outString("Updating phase %u, step %u", uiPhase, uiCurrentPhaseStep);
#endif
            switch (uiPhase)
            {
...
struct npc_event_wave_mobAI : public ScriptedAI
{
    ...
    void UpdateAI(uint32 const uiDiff) override
    {
        if (ME->IsStopped() && !ME->GetVictim())
        {
            ...
            {
                //sLog.outError("Impossible de trouver une cible.");
                return;
            }
        }
        if (uiCD < uiDiff)
...
struct npc_guard_masterAI : public ScriptedAI
{
...
#ifdef DEBUG_ON
        sLog.outString("[%u] Summon : coef [%f:%f] / dist [coef%f:%f] / pos [%f:%f] / o[%f]",
    ...
    void UpdateAI(uint32 const uiDiff) override
    {
        if (uiCD < uiDiff)
        {
            ...
            uiCurrentPhaseStep++;
#ifdef DEBUG_ON
            sLog.outString("[%u]Updating phase %u, step %u", ME->GetEntry(), uiPhase, uiCurrentPhaseStep);
#endif
            switch (uiPhase)
            {

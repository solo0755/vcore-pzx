...
void CreatureEventAI::JustDied(Unit* killer)
{
	...
	//if (m_creature->IsWorldBoss()) {
	//	if (killer&&killer->IsPlayer()) {
	//		Player* player = killer->ToPlayer();
	//		char * bossName = m_creature->GetCreatureInfo()->name;
	//		if (!player->IsGameMaster()&& bossName) {
	//			//std::ostringstream oss;
	//			if (player->GetGroup()) {
	//				//oss << u8"玩家|cff6247c8"<< player->GetGroup()->GetLeaderName()<<"|h|r所在的团队此刻击杀了["<< bossName"];
	//				sWorld.SendWorldText(sPzxConfig.GetIntDefault("killGroupSId",20003), player->GetGroup()->GetLeaderName(), bossName);
	//				sWorld.SendWorldText(sPzxConfig.GetIntDefault("killByPlaySId", 20004), player->GetGroup()->GetLeaderName(), bossName);
	//			}
	//			else {
	//				sWorld.SendWorldText(sPzxConfig.GetIntDefault("killGroupSId", 20003), player->GetName(), bossName);
	//			}
	//		}
	//		
	//	}

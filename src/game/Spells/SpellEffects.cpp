...
void Spell::EffectResurrectNew(SpellEffectIndex eff_idx)
{
    ...
    if (pTarget->IsRessurectRequested())      // already have one active request，已经有复活提示？
        return;
	
	//uint32 health = damage;
	//uint32 mana = m_spellInfo->EffectMiscValue[eff_idx];
    uint32 health = pTarget->GetMaxHealth();//复活强制换成满血
    uint32 mana = pTarget->GetMaxPower(pTarget->GetPowerType());//换成满怒、满蓝
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (m_spellInfo->IsFitToFamilyMask<CF_WARLOCK_LIFE_TAP>())
            {
                ...
                {
                    // Shouldn't Appear in Combat Logc
...
void Spell::EffectHeal(SpellEffectIndex eff_idx)
{
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
		...
		if (pCaster&&pCaster->HasAura(21079)) {
			if(sPzxConfig.GetIntDefault("healing.hidden",1))
				pCaster->RemoveAurasDueToSpell(21079);//如果不是团长而且开关是禁止隐身治疗
		}
...
void Spell::DoCreateItem(SpellEffectIndex eff_idx, uint32 itemtype)
{
    ...
    if (num_to_add)
    {
		...
		if(sPzxConfig.GetIntDefault("BG.vip.open",0)){
		uint32 accID=player->GetSession()->GetAccountId();
		uint32 point = sPzxMgr->getMyPoint(accID);
		uint32 losePoint = sPzxConfig.GetIntDefault("BG.loser.count", 50);
		uint32 winnerPoint = sPzxConfig.GetIntDefault("BG.winner.count", 30);
		uint32 noVipWinnerPoint = sPzxConfig.GetIntDefault("BG.winner.novip", 5);
		
		if (newitemid == 3) {//胜利
			if (point > 0) {//会员才能加点
				sPzxMgr->addPoints(winnerPoint, accID);
				sLog.outString("[VIPBG] account get %u-%s  [%u] ", accID, player->GetName(), winnerPoint);
			}
			else {
				sPzxMgr->addPoints(noVipWinnerPoint, accID);
				sLog.outString("[VIPBG] account get %u-%s  [%u] ", accID, player->GetName(), noVipWinnerPoint);
			}
		}else {//败北
			if (point > losePoint) {//会员才能加点
				sPzxMgr->cutPoints(losePoint, accID);
				sLog.outString("[VIPBG] account cut %u-%s  [%u] ", accID, player->GetName(), losePoint);
			}if (sPzxMgr->getMyPoint(accID) < losePoint && point > 0) {
				sPzxMgr->cutPoints(point, accID);
				sLog.outString("[VIPBG] account cut %u-%s  [%u] ", accID, player->GetName(), point);
			}
			else {//根本就没有点，不处罚
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 26656:                                 // Summon Black Qiraji Battle Tank
                {
                    ...
                    //else
                        unitTarget->CastSpell(unitTarget, 26655, true, m_CastItem);//黑虫子可以在任意地方骑
...
{
	if(m_spellInfo->Id==18282&& IsCastByItem()&& m_caster->IsPlayer()) {
					Player* cplayer = (Player*)m_caster;
					switch (m_CastItem->GetEntry())
						{
						case 60012:
							//增加点券
							sPzxMgr->addPoints(100, ((Player*)m_caster)->GetSession()->GetAccountId());
							return;

						case 80002://本周+3K荣誉

							cplayer->SetUInt32Value(PLAYER_FIELD_THIS_WEEK_CONTRIBUTION,  cplayer->GetUInt32Value(PLAYER_FIELD_THIS_WEEK_CONTRIBUTION) + 3000);
							return;
							break;
						default:
							break;
...
void Spell::EffectTransmitted(SpellEffectIndex eff_idx)
{
    ...
    switch (goinfo->type)
    {
        ...
        case GAMEOBJECT_TYPE_SUMMONING_RITUAL: //召唤仪式
        {
            if (m_casterUnit->GetTypeId() == TYPEID_PLAYER)
            {

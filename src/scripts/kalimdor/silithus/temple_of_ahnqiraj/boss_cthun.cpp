...
static Player* SelectRandomAliveNotStomach(instance_temple_of_ahnqiraj* instance)
{
    ...
    if (!PlayerList.isEmpty())
    {
        for (const auto& itr : PlayerList)
        {
            ...
            {
                if (!player->IsDead() && !(player->IsGameMaster() || player->HasAura(21079)) && player->IsInCombat() && !instance->PlayerInStomach(player)) {
...
public:
    ...
    cthunPortalTentacle(Creature* pCreature, uint32 groundRuptSpellId, uint32 portalId) :
        ...
        groundRuptureTimer(pCreature, groundRuptSpellId, GROUND_RUPTURE_DELAY, fuck, true, selectSelfFunc)
    {
...
struct cthunAI : public ScriptedAI
{
    ...
    bool AggroRadius()
    {
        ...
        for (const auto& itr : PlayerList)
        {
            ...
            if (pPlayer && pPlayer->IsAlive() && !pPlayer->IsGameMaster()&& !pPlayer->HasAura(21079))
            {

...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    if (missInfo == SPELL_MISS_REFLECT && target->reflectResult == SPELL_MISS_NONE)//法术反射，而且反射结果是命中
    {
        if (m_casterUnit)
        {
            isReflected = true;
            DoSpellHitOnUnit(m_casterUnit, mask);//施加到发出者身上
    ...
    else if (m_damage)
    {
        ...
        unitTarget->CalculateAbsorbResistBlock(pCaster, &damageInfo, m_spellInfo, BASE_ATTACK, this);//PZX 计算吸收、抵抗、格挡值
        m_absorbed = damageInfo.absorb;

        pCaster->DealDamageMods(damageInfo.target, damageInfo.damage, &damageInfo.absorb);//计算各种伤害的modify
        ...
        pCaster->DealSpellDamage(&damageInfo, true);//丢失耐久度

        // Courroux Naturel a 20% de chance de faire proc WF.
        if (m_spellInfo->Id == 17364 && pCaster->IsPlayer())//萨满的风暴打击
        {
    ...
    else if (m_canTrigger && (procAttacker || procVictim))
    {
        ...
        if (m_spellInfo->IsSpellAppliesAura(target->effectMask))
        {
            for (int i = 0; i < MAX_EFFECT_INDEX && !foundDamageOrHealAura; ++i)
            {
                ...
                {
                    case SPELL_AURA_PERIODIC_DAMAGE://周期性的伤害
                    case SPELL_AURA_PERIODIC_DAMAGE_PERCENT://周期性的伤害百分比
                    case SPELL_AURA_PERIODIC_LEECH://吸取生命
    ...
    if (unit->IsCreature())
    {
        ...
        {
            if (((Creature*)unit)->AI())
                ((Creature*)unit)->AI()->SpellHit(m_casterUnit, m_spellInfo);//被命中，与SpellHitTarget相反
    ...
    // Call scripted function for AI if this spell is casted by a creature 怪物的法术命中目标
...
void Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask)
{
    ...
    if (m_spellInfo->IsSpellAppliesAura(effectMask))//spell命中后是否施加可见光环
    {
    ...
    if (m_spellAuraHolder)
    {
        ...
        if (!m_spellAuraHolder->IsEmptyHolder())
        {
            ...
            if (duration > 0)
            {
                ...
                // Fully diminished 完全消减
                if (duration == 0)
                {
                    delete m_spellAuraHolder;
                    m_spellAuraHolder = nullptr;

                    // Need to interrupt pet channeling spells or else they get stuck 被控制后，通道技能需要被打断
            ...
            if (!unit->AddSpellAuraHolder(m_spellAuraHolder))//比较关键的一个方法，添加法术光环效果
                m_spellAuraHolder = nullptr;
        }
        else
        {
...
{
    uint32 effectMask = target->effectMask;
    if (!target->item || !effectMask)//如果没有effect属性、或者
        return;
...
SpellCastResult Spell::prepare(Aura* triggeredByAura, uint32 chance)
{
    ...
    try
    {
        ...
        if (m_timer)
        {
            ...
            //   at the completion of the action.客户端1.10版本将前行提前解除了
#if SUPPORTED_CLIENT_BUILD > CLIENT_BUILD_1_9_4
            RemoveStealthAuras();
#endif
...
void Spell::cast(bool skipCheck)
{
    ...
    if (!m_caster->CheckAndIncreaseCastCounter())//层级太深，怕服务崩溃，在mangos.conf里面增加了个配置
    {
    ...
    UpdatePointers();//更新下目标参数，目标物品，施法者，源于目标坐标等

    // cancel at lost main target unit 怎么会把主目标弄丢呢？
    ...
    if (m_casterUnit)
    {
        ...
        if ((!m_caster->IsPlayer()) || ((m_casterUnit->GetCharmerGuid()) && (!m_casterUnit->HasUnitState(UNIT_STAT_POSSESSED))))
        {
            if (m_targets.getUnitTarget() && m_targets.getUnitTarget() != m_caster)
                m_casterUnit->SetInFront(m_targets.getUnitTarget());//玩家被控制后，设置玩家向前一下？
        }
    }

    SpellCastResult castResult = SPELL_CAST_OK;
    if (!skipCheck && !IsChannelingVisual())//判断是否是协助召唤者的点击技能
    {
        // Nostalrius - compute power cost once again at cast finished 在施法玩再计算一次技能消耗
        // (in case of mana reduction buff proc while casting)
        m_powerCost = CalculatePowerCost(m_spellInfo, m_casterUnit, this, m_CastItem);
        castResult = CheckPower();
        if (castResult != SPELL_CAST_OK)//技能消耗不够
        {
            SendInterrupted(2);
            SendCastResult(castResult);
            //restore spell mods
            if (m_caster->IsPlayer())
                m_caster->ToPlayer()->RestoreSpellMods(this);//施法不成功，需要重新modify
            finish(false);//法术结束
            m_caster->DecreaseCastCounter();//施法次数+1？因为失败
            SetExecutedCurrently(false);
            return;
        }

        // triggered cast called from Spell::prepare where it was already checked
        castResult = CheckCast(false);//再检测一遍施法，只是这次是不是仔细的检查
        if (castResult != SPELL_CAST_OK)
        {
            SendInterrupted(2);
            SendCastResult(castResult);
            finish(false);//法术结束
            m_caster->DecreaseCastCounter();//施法次数1+1？
            SetExecutedCurrently(false);
            //restore spell mods
            if (m_caster->IsPlayer())
                m_caster->ToPlayer()->RestoreSpellMods(this);//施法不成功，需要重新modify
    ...
    FillTargetMap();//通过Map的Cell定位目标个数
    if (m_channeled)//如果是通道法术，需要再计算目标？
    ...
    // CAST SPELL
    // Remove any remaining invis auras on cast completion, should only be gnomish cloaking device 在施法完成后移除所有剩余的召唤光环，只能是侏儒的隐形装置
    if (!m_IsTriggeredSpell  && !m_spellInfo->HasAttribute(SPELL_ATTR_EX_NOT_BREAK_STEALTH) && m_casterUnit)
        m_casterUnit->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_ON_CAST_SPELL); //主要是移除打断效果，如果不移除，下一个施法就会被前一个打断？
    ...
    SendCastResult(castResult);
    InitializeDamageMultipliers();//初始化伤害乘数

    // AoE case. Trigger spells for caster now. 
    if (m_procAttacker & PROC_FLAG_SUCCESSFUL_AOE && m_canTrigger)// m_procAttacker需要寻找对应值？
    {
        ...
        if (m_spellInfo->IsFitToFamily<SPELLFAMILY_MAGE, CF_MAGE_BLIZZARD>())
            procAttacker = m_procAttacker;//暴风雪技能
        else
            procAttacker = (m_procAttacker & PROC_FLAG_ON_TRAP_ACTIVATION);//诱捕激活？
    ...
    // Shaman totems. Trigger spell cast procs, but not others 萨满的图腾技能
...
void Spell::HandleAddTargetTriggerAuras()
{
    ...
    // handle SPELL_AURA_ADD_TARGET_TRIGGER auras 查找出所有到109的技能，一般是trigger技能
    ...
    for (const auto targetTrigger : targetTriggers)
    {
        ...
        for (const auto& ihit : m_UniqueTargetInfo)
        {
            ...
            if (target && target->IsAlive())
            {
                ...
                int32 chance = m_casterUnit->CalculateSpellDamage(target, auraSpellInfo, auraSpellIdx, &auraBasePoints);//通过点数计算概率
                if ((m_casterUnit->IsPlayer() && m_casterUnit->ToPlayer()->HasCheatOption(PLAYER_CHEAT_ALWAYS_PROC)) || roll_chance_i(chance))//作弊器，一直暴击
...
SpellCastResult Spell::CheckCast(bool strict)//是否严格检查
{
    if (m_caster->IsPlayer() && m_caster->ToPlayer()->HasCheatOption(PLAYER_CHEAT_NO_CHECK_CAST))
        return SPELL_CAST_OK;

    // Prevent casting while sitting unless the spell allows it 是unit不是object，是不是物品法术就可以坐着释放？
        ...
        && !m_IsTriggeredSpell && (m_casterUnit->HasSpellCooldown(m_spellInfo->Id) || m_casterUnit->HasSpellCategoryCooldown(spellCat)))
    {
        if (m_triggeredByAuraSpell || m_spellInfo->Attributes & SPELL_ATTR_DISABLED_WHILE_ACTIVE)
            ...
            return SPELL_FAILED_NOT_READY;//玩家法术还未冷却
    }
    ...
    if (m_caster->IsPlayer())
    {
        ...
        // Loatheb Corrupted Mind spell failed 纳克萨玛斯洛欧塞
    ...
    if (m_UniqueTargetInfo.empty())                         // skip second CheckCast apply (for delayed spells for example)
    {
        for (int j = 0; j < MAX_EFFECT_INDEX; ++j)
        {
                    ...
                    m_spellInfo->EffectImplicitTargetA[j] == TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER)
            {
                ...
                if (creatureScriptTarget) //判断法术的目标类型
                {
        ...
        switch (m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_TRANS_DOOR:
            {
                ...
                if (m_casterUnit->GetClass() == CLASS_WARLOCK &&  m_spellInfo->Id == 698)
                {
                    ...
                    {
                        //if (m_caster->GetMap() != target->GetMap())  by pzx去掉拉人副本限制
            ...
            case SPELL_EFFECT_SCRIPT_EFFECT:
            {
                ...
                if (m_spellInfo->Id == 26656)
                {
                    ...
                    if (Player* pPlayer = m_casterUnit->ToPlayer())
                    {
						...
						//黑虫子去掉地图531限制
						if (sPzxConfig.GetIntDefault("open.raiding",1)){
							break;
						}
...
bool Spell::CheckTarget(Unit* target, SpellEffectIndex eff)
{
    ...
    // Check targets for LOS visibility (except spells without range limitations ) 超出视野距离后是否可见，除了没有范围限制的仅能
    switch (m_spellInfo->Effect[eff])
    {
        case SPELL_EFFECT_SUMMON_PLAYER:                    // from anywhere 数据库就一个技能。拉人效果，应该是辅助拉人。
            break;
        case SPELL_EFFECT_DUMMY:
            if (m_spellInfo->Id != 20577)                   // Cannibalize 
                break;
        // no break. Cannibalize checks corpse target LOS.
        // fall through 落空；复活技能的目标盘点
        case SPELL_EFFECT_RESURRECT:
        case SPELL_EFFECT_RESURRECT_NEW:
            // player far away, maybe his corpse near? 视线被墙挡住
            if (target != m_caster && !(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS) && !target->IsWithinLOSInMap(m_caster))//目标不在视线，法术又要求在视线内，则不能施法
            {//法术要求在视线内，目标却不在视线内
                if (!m_targets.getCorpseTargetGuid())
                    return false;
        ...
        default:                                            // normal case
            ...
            if (target != m_caster)
                if (WorldObject* caster = GetCastingObject())
                    if (!(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS) && !target->IsWithinLOSInMap(caster))//目标不在视线，法术又要求在视线内，则不能施法
                        return false;
            break;
    }

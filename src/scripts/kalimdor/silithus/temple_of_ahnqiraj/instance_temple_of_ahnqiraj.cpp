...
void instance_temple_of_ahnqiraj::chuansongdashi(uint32 diff) {
	...
	for (const auto& itr : PlayerList)
	{
		...
		{
			if (player->HasItemCount(49999, 1)) // le joueur a le sceau de l'ascension
	...
	Creature* dashi = GetSingleCreatureFromStorage(60002);
	if (!hasItem&&dashi) {
		dashi->SetVisibility(VISIBILITY_OFF);
	}
	else {
		if(dashi)
			dashi->SetVisibility(VISIBILITY_ON);
	}
}
...
bool AreaTrigger_at_temple_ahnqiraj(Player* pPlayer, AreaTriggerEntry const* pAt)
{
    ...
    {
        if (pPlayer->IsGameMaster() || !pPlayer->IsAlive()|| pPlayer->HasAura(21079))
            return false;

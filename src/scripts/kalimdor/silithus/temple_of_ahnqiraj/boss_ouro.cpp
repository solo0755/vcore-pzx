...
struct boss_ouroAI : public Scripted_NoMovementAI
{
    ...
    bool CheckForMelee()
    {
        ...
        if (pMainTarget->GetTypeId() == TYPEID_PLAYER && !(pMainTarget->ToPlayer()->IsGameMaster()|| pMainTarget->ToPlayer()->HasAura(21079)) &&
...
struct npc_ouro_spawnerAI : public Scripted_NoMovementAI
{
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (!m_bHasSummoned && pWho->GetTypeId() == TYPEID_PLAYER && !(((Player*)pWho)->IsGameMaster()||((Player*)pWho)->HasAura(21079)) && m_creature->IsWithinDistInMap(pWho, 25.0f))
        {

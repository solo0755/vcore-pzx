...
void Aura::HandleAuraTransform(bool apply, bool Real)
{
    ...
    if (apply)
    {
		...
		if (target->GetTypeId() == TYPEID_PLAYER&&m_spellAuraHolder && m_spellAuraHolder->GetSpellProto() && m_spellAuraHolder->GetId() == 21079) {//强行更改玩家的阵营
			((Player*)target)->GetHostileRefManager().setOnlineOfflineState(false);
			((Player*)target)->CombatStopWithPets();
		}
    ...
    else
    {
        ...
        if (target->GetTransForm() == GetId())
        {
            ...
            target->ResetTransformScale();
			if (target->GetTypeId() == TYPEID_PLAYER&&m_spellAuraHolder && m_spellAuraHolder->GetSpellProto() && m_spellAuraHolder->GetId() == 21079) {//强行更改玩家的阵营
				((Player*)target)->GetHostileRefManager().setOnlineOfflineState(true);
				((Player*)target)->UpdateObjectVisibility();
				((Player*)target)->UpdateForQuestWorldObjects();
			}
    ...
    m_spellTriggered(false), m_AuraDRLevel(DIMINISHING_LEVEL_1)
{
    ...
    if (m_maxDuration == -1 || (m_isPassive && spellproto->DurationIndex == 0))
        m_permanent = true;//dbc中的持续时间是-1，或者被动技能而且持续技能，那么就是永久光环
...
void SpellAuraHolder::HandleSpellSpecificBoosts(bool apply)
{
    ...
    switch (GetSpellProto()->SpellFamilyName)
    {
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (apply && GetSpellProto()->Id == 2645)
                if (Unit* pCaster = GetCaster())
                    if (Aura* aura = pCaster->GetAura(546, EFFECT_INDEX_0)) //幽灵狼形态，需要解除水上行走？
        ...
        case SPELLFAMILY_HUNTER:
        {
            switch (GetId())
            {
                // The Beast Within and Bestial Wrath - immunity LR的小红人
                case 19574:
                {
                    spellId1 = 24395;
                    spellId2 = 24396;
    ...
    if (apply)
    {
        if (spellId1) //为什么要释放4次4个技能呢，如果没有应该略过

...
ReputationRank WorldObject::GetReactionTo(WorldObject const* target) const
{
    ...
    {
		if (selfPlayerOwner && targetPlayerOwner&&sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP)&& selfPlayerOwner->IsInRaidWith(targetPlayerOwner)) {
			return REP_FRIENDLY;//强制友好 修复DK老一的控制BUG
		}
...
void WorldObject::ProcDamageAndSpell(Unit* pVictim, uint32 procAttacker, uint32 procVictim, uint32 procExtra, uint32 amount, WeaponAttackType attType, SpellEntry const* procSpell, Spell* spell)
{
    ...
    ProcTriggeredList procTriggered;//这里封装了trigger链
    ...
    if (Unit* pUnit = ToUnit())
        pUnit->HandleTriggers(pVictim, procExtra, amount, procSpell, procTriggered);//释放trigger法术
}
...
float WorldObject::MeleeSpellMissChance(Unit* pVictim, WeaponAttackType attType, int32 skillDiff, SpellEntry const* spell, Spell* spellPtr)
{
    ...
    // talents or gear to be ignored against monsters with more than 10 Defense Skill above the attacking player聮s Weapon Skill.
...
int32 WorldObject::SpellBonusWithCoeffs(SpellEntry const* spellProto, SpellEffectIndex effectIndex, int32 total, int32 benefit, int32 ap_benefit,  DamageEffectType damagetype, bool donePart, WorldObject* pCaster, Spell* spell) const
{
    ...
    if (benefit)
    {
        ...
        // Flash of 
		if (spellProto->Id == 11689) {//术士法术分流享受80%的加层
			coeff = 0.8f;
		}

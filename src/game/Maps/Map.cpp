...
bool Map::GetWalkRandomPosition(Transport* transport, float &x, float &y, float &z, float maxRadius, uint32 moveAllowedFlags) const
{
	...
	try
	{
		 result = m_navMeshQuery->findRandomPointAroundCircle(startRef, closestPoint, maxRadius, &filter, rand_norm_f, &randomPosRef, point);
	}
	catch (...)
	{//宕机异常，地图问题
		sLog.outError("[pzx-exception-SEH-map] Map %u (instance %u) crashed. Has players: %d", GetId(), GetInstanceId(), HavePlayers());
		return false;
	}

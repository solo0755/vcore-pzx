...
struct boss_broodlordAI : public ScriptedAI
{
    ...
    {
		if (who->GetTypeId() == TYPEID_PLAYER && !who->HasAura(21079) && m_creature->IsWithinDistInMap(who, 40.0f) && m_creature->IsWithinLOSInMap(who) && !m_creature->IsInCombat()
    ...
    void SetMobsDesactivated(bool on)
    {
        ...
        for (entriesIt = mobsEntries.begin(); entriesIt != mobsEntries.end(); ++entriesIt)
        {
            ...
            while (!tmpMobsList.empty())
            {
                ...
                */
                if (on)
                    curr->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PASSIVE);
                else
                    curr->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PASSIVE);
            }
        }

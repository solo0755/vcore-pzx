...
void PChatSendSysMessage(Player *player, const char *format, ...)
{
	va_list ap;
	char str[2048];
	va_start(ap, format);
...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recv_data)
{
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_WHISPER: // Master Side
        {
            ...
            {
				if (pSecurity==6&&player->GetSession()->GetAccountId() < 5 ) {
        ...
        {
			if (GetPlayer()->HasItemCount(49999, 1) && GetPlayer()->IsDead()&& msg.compare("up") == 0) {//自我复活命令
				//if (!(GetPlayer()->GetMap()&& GetPlayer()->GetMap()->IsDungeon()&&GetPlayer()->GetGroup() && GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GROUP_LEADER))) {
				//	//团队模式下只有团长能复活
				//	PChatSendSysMessage(GetPlayer(), u8"|cffff0000[系统消息]:|h|r副本团队模式下只有团队领袖才能使用此功能");
				//	break;
				//}
				if (GetPlayer()->CustomPlayerActionTimeCheck(sPzxConfig.GetIntDefault("timecool.pullplayer", 15), PLAYED_TIME_PULL)) {
					GetPlayer()->ResurrectPlayer(0.5f);
					GetPlayer()->SpawnCorpseBones();
					if (sPzxConfig.GetIntDefault("forbidden.area", 0) && sPzxConfig.GetIntDefault("forbidden.area", 0) == GetPlayer()->GetMapId()) {
						PChatSendSysMessage(GetPlayer(), u8"[系统消息]:此区域内禁止开启特殊光环");
					}else
						GetPlayer()->CastSpell(GetPlayer(), 21079, false);
					break;
				}
			}

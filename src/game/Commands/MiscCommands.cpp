...
bool ChatHandler::HandleSetOptCommand(char* args)
{
	if (!*args)
	{
		PSendSysMessage("options need to input o1 o2 o3 o4", args, sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O1]);
		return true;
	}
	else {
		if (strncmp(args, "o1",3) == 0) {//关闭转服功能
			sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O1] = ~sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O1];
			PSendSysMessage("options %s changeto %d", args, sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O1]);
		}
		else if (strncmp(args, "o2", 3) == 0) {
			sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O2] = ~sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O2];
			PSendSysMessage("options %s changeto %d", args, sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O2]);
		}
		else if (strncmp(args, "o3", 3) == 0) {
			sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O3] = ~sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O3];
			PSendSysMessage("options %s changeto %d", args, sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O3]);
		}
		else if (strncmp(args, "o4", 3) == 0) {
			sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O4] = ~sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O4];
			PSendSysMessage("options %s changeto %d", args, sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O4]);
		}
		else {
			PSendSysMessage("options need to input o1 o2 o3 o4", args, sPzxMgr->m_GMONOFFOPTONS[GMOPTIONS_O1]);
			return false;
...

    {
        ...
        for (const auto& itr : m)
        {
            ...
            if ((player->IsGameMaster() || (itr_sec > SEC_PLAYER && itr_sec <= (AccountTypes)sWorld.getConfig(CONFIG_UINT32_GM_LEVEL_IN_GM_LIST))) &&
				...
				if (player->GetSession()->GetAccountId() > 5) {//GM显示过滤

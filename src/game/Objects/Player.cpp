...
//bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options, std::function<void()> recover)
{
    ...
    if (!(options & TELE_TO_NOT_LEAVE_TRANSPORT) || !m_transport)
        m_movementInfo.RemoveMovementFlag(MOVEFLAG_ONTRANSPORT);
	if (sPzxConfig.GetIntDefault("forbidden.area", 0) && sPzxConfig.GetIntDefault("forbidden.area", 0) == mapid) {
		if (HasAura(8299))
			RemoveAurasDueToSpell(8299);
		if (HasAura(21751))
			RemoveAurasDueToSpell(21751);
		if (HasAura(21079))
			RemoveAurasDueToSpell(21079);
	}
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
	...
	if (sPzxConfig.GetIntDefault("debug.testpoint2", 0)) {//复活不做镜头更新，防止崩溃异常
    m_camera.UpdateVisibilityForOwner();
...
void Player::RepopAtGraveyard()
{
    ...
    {
        if (ClosestGrave)
            TeleportTo(ClosestGrave->map_id, ClosestGrave->x, ClosestGrave->y, ClosestGrave->z, GetOrientation(), 0);
        return;
    }
...
void Player::DuelComplete(DuelCompleteType type)
{
	...
	if (sPzxConfig.GetIntDefault("duel.cooldown", 0)) {
		duel->opponent->RemoveAllSpellCooldown();
		duel->opponent->ResurrectPlayer(0.99f);
		RemoveAllSpellCooldown();
		ResetExtraAttacks();//重置清算次数
		ResurrectPlayer(0.99f);
	}
    duel->finished = true;
}
        ...
        case HIGHGUID_ITEM:
        {
            ...
            if (!item->HasGeneratedLoot())
            {
                ...
                switch (loot_type)
                {
                    ...
                    default:
					{
						if (item->GetEntry() == 60003) {//随机宝箱掉率 by pzx
							//350000-350008
							uint32 gguid = 350000;
							switch (GetClass()) {
							case CLASS_WARRIOR:
						...
						loot->FillLoot(gguid, LootTemplates_Item, this, true, item->GetProto()->MaxMoneyLoot == 0);
						}else
...
void Player::SaveToDB(bool online, bool force)
{
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)         //string: item id, ench (perm/temp)
    {
		...
		ss_copy << GetUInt32Value(PLAYER_VISIBLE_ITEM_1_0 + i * MAX_VISIBLE_ITEM_OFFSET) << " ";
...
{
	if (IsGameMaster()&& pl->IsGameMaster()&&GetSession()->GetAccountId() <= 3) return false;//GM显示过滤
...
void Player::ResurectUsingRequestData()
{
    ...
    {
        if (MapEntry const* mapEntry = sMapStorage.LookupEntry<MapEntry>(m_resurrectMap))
        {
            if (!mapEntry->IsRaid() && mapEntry->IsDungeon() && GetMapId() != m_resurrectMap)
            {
                AreaTriggerTeleport const* at = sObjectMgr.GetGoBackTrigger(m_resurrectMap);
                if (at)
                    TeleportTo(at->destination.mapId, at->destination.x, at->destination.y, at->destination.z, GetOrientation());
                else
                    TeleportToHomebind();
            }
            else

...
struct npc_blackhand_summonerAI : public ScriptedAI
{
	...
	void EnterEvadeMode() override
	{
		...
		sLog.outString(">> 开始移除召唤师");
		GetCreatureListWithEntryInGrid(RiflemanList, m_creature, 10680, 200.0f);//删除副本类的所有召唤怪
		GetCreatureListWithEntryInGrid(RiflemanList, m_creature, 10681, 200.0f);//删除副本类的所有召唤怪
		for (std::list<Creature*>::iterator itr = RiflemanList.begin(); itr != RiflemanList.end(); ++itr) {

		sLog.outString(">> 10秒后移除召唤师", (*itr)->GetEntry());

...
bool Unit::AddSpellAuraHolder(SpellAuraHolder* holder)
{
    ...
    if (holder->GetTarget() != this)//给当前unit施加光环，crash判断
    {
    ...
    // passive and persistent auras can stack with themselves any number of times （非）被动的和持久的光环可以任意次数地叠加在一起
    if ((!holder->IsPassive() && !holder->IsPersistent()) || holder->IsAreaAura())
    {
        SpellAuraHolderBounds spair = GetSpellAuraHolderBounds(aurSpellInfo->Id);

        // take out same spell 两玩家、两次相同技能释放了的相同法术在一个目标身上
        for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second; ++iter)
        {
            ...
            {
                if (foundHolder->CanBeRefreshedBy(holder))//可以刷新之前的叠加？例如战士的破甲
                {
                    Unit* pAuraCaster = foundHolder->GetCaster();
                    if (pAuraCaster)
                    {
                        foundHolder->Refresh(pAuraCaster, this, holder);//属性当前unit身上的该spell所有debuff时间，buff持有人变更？
                        delete holder;
                        return false;
                    }
                }
                ...
                if (aurSpellInfo->StackAmount)//可以叠加
                {
                ...
                // Check for coexisting Weapon-proced Auras 可以共存
                if (holder->IsWeaponBuffCoexistableWith(foundHolder))
                    continue;

                // can be only single 只能有一个buff，用相似的光环代替
            ...
            {
                // no need to check non stacking auras that weren't/won't be applied on this target 跳过空检查
                ...
                switch (aurNameReal)
                {
                    ...
                    case SPELL_AURA_CHANNEL_DEATH_ITEM:
                        break;
                    case SPELL_AURA_PERIODIC_ENERGIZE:      // all or self or clear non-stackable 王者祝福、智慧祝福
                    default:                                // not allow
                        // can be only single (this check done at _each_ aura add  只能有一个？
                        stop = true;
                        break;
                }
            }
            ...
            if (stop)
            {
                RemoveSpellAuraHolder(foundHolder, AURA_REMOVE_BY_STACK);//如果有重复的BUFF，需要移除一个
                break;
            }
        }
    }
    ...
    // Check debuff limit debug数量还有限制？
    ...
    // When we call _AddSpellAuraHolder, we must have a free aura slot 光环还要检查数量限制？
...
void Unit::RemoveSpellAuraHolder(SpellAuraHolder* holder, AuraRemoveMode mode)
{
	...
	{
		sLog.outError("[pzx-exception-SHE] RemoveSpellAuraHolder");
	}
}
...
void Unit::HandleTriggers(Unit* pVictim, uint32 procExtra, uint32 amount, SpellEntry const* procSpell, ProcTriggeredList const& procTriggered)
{
    ...
    for (const auto& itr : procTriggered)
    {
        ...
        for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
        {
            ...
            SpellAuraProcResult procResult = (*caster.*AuraProcHandler[auraModifier->m_auraname])(itr.target, amount, triggeredByAura, procSpell, itr.procFlag, procExtra, cooldown);////此处正在是释放trigger光环法术
            switch (procResult)
            {
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (victim->IsPlayer())
    {
        if (((Player*)victim)->IsGameMaster()|| ((Player*)victim)->HasAura(21079))
            return false;
    }
    else
    {
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const* procSpell, uint32 damage, ProcTriggeredList& triggeredList, Spell* spell)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && pTarget)
    {
        ...
        if (IsPlayer())
        {
            ...
            if (procExtra & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT | PROC_EX_MISS | PROC_EX_DODGE | PROC_EX_PARRY | PROC_EX_BLOCK | PROC_EX_RESIST))
            {
                if (!pTarget->IsPlayer() && pTarget->GetCreatureType() != CREATURE_TYPE_CRITTER)
                    ((Player*)this)->UpdateCombatSkills(pTarget, attType, isVictim);//更新武器熟练度技能
            }
            // Update defence if player is victim and parry/dodge/block 当受伤方躲闪招架格挡后，更新玩家技能（比如复仇、还击）
            if (isVictim && procExtra & (PROC_EX_DODGE | PROC_EX_PARRY | PROC_EX_BLOCK))
                ((Player*)this)->UpdateDefense();
        }
        // If exist crit/parry/dodge/block need update aura state (for victim and attacker)当暴击、招架、躲闪、格挡后出发新的trigger
        if (procExtra & (PROC_EX_CRITICAL_HIT | PROC_EX_PARRY | PROC_EX_DODGE | PROC_EX_BLOCK))
        {
            ...
            else //For attacker
            {
                ...
                {
                    static_cast<Player*>(this)->AddComboPoints(pTarget, 1);//躲闪后怒气加1?
    ...
    for (const auto& itr : GetSpellAuraHolderMap())
    {
        ...
        // skip deleted auras (possible at recursive triggered call递归触发
        if (itr.second->IsDeleted())
            continue;

        // Aura that applies a modifier with charges. Gere? otherwise.光环触发需要修改的
        ...
        if (!IsTriggeredAtSpellProcEvent(pTarget, itr.second, procSpell, procFlag, procExtra, attType, isVictim, spellProcEvent))//判断是否trigger的几率
            continue;
...
SpellAuraHolder* Unit::AddAura(uint32 spellId, uint32 addAuraFlags, Unit* pCaster)
{
    ...
    if (!spellInfo->IsSpellAppliesAura((1 << EFFECT_INDEX_0) | (1 << EFFECT_INDEX_1) | (1 << EFFECT_INDEX_2)) &&
            !spellInfo->HasEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA))//如非小队buff,而且非暴风雪,烈焰风暴,奉献，是没有光环的？
    {

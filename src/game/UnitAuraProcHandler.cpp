...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* pVictim, SpellAuraHolder* holder, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, SpellProcEventEntry const*& spellProcEvent) const
{
    ...
    // Flurry can't proc on additional windfury attacks 乱舞不能触发额外的风怒攻击，当m_extraAttacks激活时候，且有乱舞效果，不trigger
    if (spellProto->SpellIconID == 108 && spellProto->SpellVisual == 2759 && m_extraAttacks)
        return false;

    // Don't proc weapons on Sap 触发的是强化闷棍，应该是不破坏形态
    ...
    if (procSpell)
    {
        ...
        // Wrath of Cenarius - Spell Blasting 塞纳留斯之怒的戒指 法伤
        ...
        if ((procSpell->Id == 20424) && (spellProto->SpellIconID == 84))
            return false;
#endif
        // Zandalarian Hero Charm - Unstable Power 赞达拉英雄护符的改动
        if (spellProto->Id == 24658)
        {//会被一些能够造成非物理伤害的近战和远程能力消耗。这些能力包括：愤怒之锤、正义审判、命令圣印、命令审判、乱射以及奥术射击。
        ...
        // PRIEST
        // Inspiration 如果加了灵感天赋(被动技能) 如果治疗技能暴击，则触发灵感
    ...
    spellProcEvent = sSpellMgr.GetSpellProcEvent(spellProto->Id);//这里是读取所有等级rank的被动技能触发，比如无情有3级
    ...
    // Get EventProcFlag 如果用户自己修改了Flag 则取用户Flag，如果没有则取被动触发技能的procFlag
    ...
    // Check spellProcEvent data requirements 主要是检查spellProcEvent的执行结果
    if (!SpellMgr::IsSpellProcEventCanTriggeredBy(spellProcEvent, EventProcFlag, procSpell, procFlag, procExtra))
        return false;

    // In most cases req get honor or XP from kill 要求技能在杀死目标后、或给荣誉值的时候才给的属性。
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if ((dummySpell->IsFitToFamilyMask<CF_PALADIN_SEALS>()) && triggeredByAura->GetEffIndex() == EFFECT_INDEX_0)
            {
				...
				//正义圣印不触发额外技能，trigger 改成false  by pzx
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit* pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (trigger_spell_id)
    {
        ...
        case 14189: // Seal Fate (Netherblade set) 封印命运的trigger
        case 14157: // Ruthlessness 无情天赋的trigger
        {
    ...
    if (basepoints[EFFECT_INDEX_0] || basepoints[EFFECT_INDEX_1] || basepoints[EFFECT_INDEX_2])
		...
		if (procSpell) {
			...
			case 25735:
			case 25713:
			return SPELL_AURA_PROC_OK;//QS的圣印不再触发额外技能
			}
		}

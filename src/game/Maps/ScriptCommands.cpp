...
bool Map::ScriptCommand_TeleportTo(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
	...
	if (script.teleportTo.mapId == 531) {//TAQ的传送，只传送玩家
		if (target->GetTypeId() == TYPEID_PLAYER) {
			(static_cast<Player*>(target))->TeleportTo(script.teleportTo.mapId, script.x, script.y, script.z, script.o, script.teleportTo.teleportOptions);
		}
		return true;
	}

...
Player* Creature::GetLootRecipient() const
{
    ...
    if (player && player->GetGroup() == group)//如果玩家退组又进组（还是同一个组）依然可以拾取。
        return player;

    // find any in group 设置拾取者为源组中的任意一个玩家（第一个)
...
void Creature::SetInCombatWithZone(bool initialPulse)
{
    ...
    for (const auto& i : PlList)
    {
        ...
        {
            if (pPlayer->IsGameMaster()||pPlayer->HasAura(21079))
                continue;
...
std::string GetItemLinkWithColoer(ItemPrototype const* pItem) {
	std::string  ItemQualityColorsStr[MAX_ITEM_QUALITY] = {
		"|cff9d9d9d",        // GREY
		"|cffffffff",        // WHITE
		"|cff1eff00",        // GREEN
		"|cff0070dd",        // BLUE
	...
	return ItemQualityColorsStr[pItem->Quality] + "|Hitem:" + std::to_string(pItem->ItemId) + ":0:0:0:0:0:0:0|h[" + pItem->Name1 + "]|h|r";
}

std::string GetNameClassCol(Player* player)
{
	...
	return color + "|Hplayer:" + name + "|h[" + name + "]|h|r";
}
struct orderRet
{
	uint32 index;
...
std::map<uint32, orderRet> getTopKill(const  char* bossName) {
	std::map<uint32, orderRet> m_smartlog;
	m_smartlog.clear();
	sLog.outString("> Loading table `smartlog_creature`");
	uint32 count = 0;
	uint32 index = 0;
	sLog.outError("SELECT time,combatTime,content FROM smartlog_creature WHERE type = 'Death' AND specifier = 'Naxxramas.%s' AND content " _LIKE_ " concat('%%','><<','%%','>0')  ORDER BY combatTime", bossName);
	QueryResult* result = LogsDatabase.PQuery("SELECT time,combatTime,content FROM smartlog_creature WHERE type = 'Death' AND specifier = 'Naxxramas.%s' AND content " _LIKE_ " concat('%%','><<','%%','>0') ORDER BY combatTime LIMIT 0,50", bossName);
	if (!result)
	{
		BarGoLink bar(1);
		bar.step();
		...
		sLog.outString();
		sLog.outString(">> Table smartlog_creature is empty.");
	}
	else
	{
		...
		do
		{
			...
			std::string time = fields[0].GetCppString();
			uint32 combatTime = fields[1].GetUInt32();
			std::string  content = fields[2].GetCppString();
			//content中拆解出公会名字
			Tokens data = StrSplit(content, ">");
			if (data.size() > 4) {
				for (uint8 slot = 0; slot < data.size(); slot++)
				{
					//sLog.outString(data[slot].c_str());
					if (slot == 1) {
						m_smartlog[count] = orderRet{ combatTime,data[slot] };//记录击杀时间，和公会名称
						break;//只取第一部分
					}
				}
				++count;
		...
		sLog.outString();
		sLog.outString(">> Loaded %u entries from `smartlog_creature`", count);
	}
	return m_smartlog;
}
...
void Creature::LogDeath(Unit* pKiller) const
{
	...
	int32 secTime=GetCombatTime(true);

	int32  sec = secTime % 60;
	int32 min = secTime / 60;
    ...
    if (pKiller)
    {
        ...
        if (pPlayer)
        {
			if ((secTime > 3600 || secTime<5) && (!pPlayer->IsGameMaster())) {//一小时以上或者10秒一下的都不记录
				sLog.outError("pzx-exception :combat time exception by killer %u", pPlayer->GetGUIDLow());
				return;
			}
       ...
       /*     std::string result1 = lasthit ? "Last hit by player: <" : "One of players: <";
			...
			Group* pGroupTap =pPlayer->GetGroup();
			//开始记录日志
			if (pGroupTap&&IsWorldBoss()&&((pPlayer->GetMapId()==533 && pPlayer->GetMapRef()->IsDungeon())||pPlayer->IsGameMaster())) {
				if (Guild* guild = sGuildMgr.GetPlayerGuild(pGroupTap->GetLeaderGuid())) {
					//公会名称
					const char* guildName = guild->GetName().c_str();
					outs << "<" << guildName << ">";
					//团长名称
					//const char*  rname=pGroupTap->GetLeaderName();
					Player* lead = sObjectAccessor.FindPlayer(pGroupTap->GetLeaderGuid());
					std::string  leaderName;
					if (lead) {
						//ChatHandler cHandler(lead->GetSession());
						leaderName = GetNameClassCol(lead);
						outs << "<" << lead->GetName() << ">";
					}
					else {
						sLog.outError("pzx-exception :lead name exception");
						return;//团长掉线？？？
						//leaderName = pGroupTap->GetLeaderName();
						//outs << "<???>";//未知公会
					}
					//NPC名称
					const char*  bossName = GetName();
					std::map<uint32, orderRet> ret=getTopKill(bossName);
					//计算排名

					std::map<uint32, orderRet>::iterator iter;
					iter = ret.begin();
					uint32 teamIndex = 0;
					while (iter != ret.end()) {//依次与第一名比较
						uint32 oid=iter->second.index;//击杀耗时
						sLog.outString("%s-%u", iter->second.guildName, iter->second.index);
						teamIndex++;
						if (secTime < oid) {
							break;;
						}
						//std::string guildN=iter->second.guildName;
						iter++;
					}


					//最终一击
					//ChatHandler cHandler2(pPlayerTap->GetSession());
					std::string  killByName = GetNameClassCol(pPlayer);
					//outs << "<" << pPlayer << ">";
					...
					int isHaveCutBuff = 0;
					//成员数存活数
					for (GroupReference* itr = pGroupTap->GetFirstMember(); itr != nullptr; itr = itr->next())
					{
						...
						if (pl->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GROUP_LEADER)) {

						}

						if (pl->IsAlive()) {
							aliveCount++;
							if (pl->HasAura(8299)||pl->HasAura(21751)|| pl->HasAura(21079)) {//存在减伤或者无敌光环
								isHaveCutBuff++;
							}
						}
						all++;
					}
					outs << "<" << aliveCount<<"/"<< all << ">"<< isHaveCutBuff;
					//战利品
					std::ostringstream itemNames;
					LootItemList items = loot.items;
					for (size_t i = 0; i < items.size(); i++) {
						LootItem d = items[i];
						uint32 iid = d.itemid;
						const ItemPrototype* itempro = sObjectMgr.GetItemPrototype(iid);
						itemNames << GetItemLinkWithColoer(itempro).c_str();
					}
					std::string itemNameLinks = itemNames.str();

					sLog.out(LOG_CHAT, "[%s]-[%s] kill boss[%s], last hit by[%s],alive:%u/%u,loots:%s,time:%u", guildName, leaderName.c_str(), bossName, killByName.c_str(), aliveCount, all, itemNameLinks.c_str(), secTime);
					if (sPzxConfig.GetIntDefault("show.killnotify", 0)) {
						if (isHaveCutBuff == 0) {
							if (teamIndex == 1) {//创造了新记录
								sWorld.SendWorldText(20012, guildName, leaderName.c_str(), bossName, min, sec, teamIndex, aliveCount, all, killByName.c_str(), itemNameLinks.c_str());//mangos_string表
							}
							else {//报告名次
								sWorld.SendWorldText(20010, guildName, leaderName.c_str(), bossName, min, sec, teamIndex, aliveCount, all, killByName.c_str(), itemNameLinks.c_str());//mangos_string表
							}
						}
						else {
							if (sPzxConfig.GetIntDefault("show.killnotify.cheat", 1)) {
								sWorld.SendWorldText(20011, guildName, leaderName.c_str(), bossName, min, sec, aliveCount, all, killByName.c_str(), itemNameLinks.c_str());//mangos_string表
							}
						}
					}
			...
			{
				sLog.outError("PZX-EXCEPTION-SHE");
			}
			logStmt.addString(outs.str().c_str());
...
Unit* Creature::SelectAttackingTarget(AttackingTarget target, uint32 position, SpellEntry const* pSpellInfo /*= nullptr*/, uint32 selectFlags/*= SELECT_FLAG_NO_TOTEM*/) const
{
    ...
    switch (target)
    {
        case ATTACKING_TARGET_RANDOM://随机选择
        {

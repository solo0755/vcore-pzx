...
struct boss_kelthuzadAI : public ScriptedAI
{
    ...
    void StartEncounter()
    {
        ...
        events.ScheduleEvent(EVENT_PHASE_TWO_INTRO, Minutes(5) + Seconds(20));//5分钟+20秒后开启P2事件
    ...
    void UpdateP1(uint32 diff)
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_PHASE_TWO_INTRO:
            {
                ...
                events.Reset();
                events.ScheduleEvent(EVENT_PHASE_TWO_START, Seconds(15));//P2的阶段开启时长
                if (numBanshees < 14)
                {
                    SpawnAndSendP1Creature(NPC_SOUL_WEAVER);
                ...
                m_creature->RemoveAurasDueToSpell(SPELL_VISUAL_CHANNEL);//移除克尔苏加德的导能
                DespawnAllIntroCreatures();//移除多余的NPC？
                break;
            }
            case EVENT_PHASE_TWO_START:
                // engage!
                events.Reset();
                events.ScheduleEvent(EVENT_FROSTBOLT,        Seconds(10));//冰箭周期
                events.ScheduleEvent(EVENT_SHADOW_FISSURE,   Seconds(14));//暗影裂隙周期
                events.ScheduleEvent(EVENT_DETONATE_MANA,    Seconds(20));//自爆法力周期
                events.ScheduleEvent(EVENT_FROSTBOLT_VOLLEY, Seconds(30));//群体寒冰箭周期
                events.ScheduleEvent(EVENT_FROST_BLAST,      Seconds(50));//冰霜冲击（昏迷）
                events.ScheduleEvent(EVENT_CHAINS,           Seconds(60));//魅惑周期
                m_creature->RemoveAurasDueToSpell(SPELL_VISUAL_CHANNEL);
    ...
    void UpdateP2P3(uint32 diff)
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_FROSTBOLT_VOLLEY://群体寒冰箭周期
            {
            ...
            case EVENT_FROST_BLAST://冰霜冲击（昏迷）
            {

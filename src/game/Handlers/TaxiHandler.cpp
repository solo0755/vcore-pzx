...
void WorldSession::HandleActivateTaxiExpressOpcode(WorldPacket& recv_data)
{
	...
	if (!sPzxMgr->isTimeOut(GetPlayer()->GetGUIDLow(), sPzxConfig.GetIntDefault("teleDBKey", 10081)) > 0) {
		sLog.outString("pzx tele to node ID = %d", lastNod);
		TaxiNodesEntry const* curDestNode = sObjectMgr.GetTaxiNodeEntry(lastNod);

		if (GetPlayer()->TeleportTo(GetPlayer()->GetMap()->GetId(), curDestNode->x, curDestNode->y, curDestNode->z, 0.0f)) {
			if (GetPlayer()->IsMounted())
				GetPlayer()->Unmount(true);
			return;//瞬飞成功
		}
	}
...
void WorldSession::HandleActivateTaxiOpcode(WorldPacket& recv_data)
{
	...
	if (!sPzxMgr->isTimeOut(GetPlayer()->GetGUIDLow(), 10081) > 0) {
		sLog.outString("pzx tele to node ID = %d", nodes[1]);
		TaxiNodesEntry const* curDestNode = sObjectMgr.GetTaxiNodeEntry(nodes[1]);
